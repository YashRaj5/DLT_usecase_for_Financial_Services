...
# MAGIC %md
# MAGIC The code below produces data using Python and saves files in JSON format to a cloud Kafka instance. In particular, there are 2 topics written to:
# MAGIC * `purchase_trends`
# MAGIC * `checking_account`
# MAGIC
...
# DBTITLE 1,Creating Database for Data Storing
# MAGIC %sql
# MAGIC CREATE DATABASE IF NOT EXISTS banking;
# MAGIC USE banking;

# COMMAND ----------

# MAGIC %md
# MAGIC ### The code below creates a shell [Debezium](https://debezium.io/) record which is parameterized so we can create new records
# MAGIC <p></p>
# MAGIC
# MAGIC * Create a template record with customer attributes parameterized
# MAGIC * Create 200 json files, one per time event with new buying patterns changing over time
# MAGIC * Create ending point - purchase of a mortage
# MAGIC * Write to kafka in json format

# COMMAND ----------

# DBTITLE 1,Cleaning up data path
# MAGIC %fs rm -r /home/fs/banking_personalization

# COMMAND ----------

# DBTITLE 1,Making the data path
# MAGIC %fs mkdirs /home/fs/banking_personalization

# COMMAND ----------

dbutils.fs.ls("dbfs:/home/fs/")

# COMMAND ----------

# MAGIC %md
# MAGIC ### Creating a Tempelate Debezium Record
...
# Create your a Kafka topic unique to your name
...
# DBTITLE 1,Defining UDF for UUID
...
input_stream = (spark
    ...
    .json(input_path)
    .withColumn("processingime", lit(datetime.now().timestamp()).cast("timestamp"))
...
# DBTITLE 1,Writing Stream to Kafka
...
# DBTITLE 1,Staging data in Delta Table
...
# DBTITLE 1,Writing Data to Kafka
# For the sake of an example, we will write to the Kafka servers using SSL/TLS encryption
# Hence, we have to set the kafka.security.protocol property to "SSL"
